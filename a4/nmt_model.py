...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        # 这里的参数解释官网看不明白，我看了一下结果，可以不定义batch_size,
        # 我们直接将知道的Parameters都赋值一遍
        # 其中hidden_size就是实际sequence的长度
        # 默认的这里就省略了，包括：num_layers=1, bias=True
        self.encoder = nn.LSTM(input_size=embed_size, hidden_size=hidden_size, bidirectional=True)
        # 这里要注意加上hidden_size，decoder是将其concat一起做
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        # 对dec_hidden unsqueezing，从(b,h)变为(b,h,1)
        dec_hidden_uns = torch.unsqueeze(dec_hidden, 2)
        e_t = torch.bmm(enc_hiddens_proj, dec_hidden_uns) # 得到(b,src_len,1)
        e_t = torch.squeeze(e_t, dim=2)  # 指定住dim
        ...
        alpha_t_uns = torch.unsqueeze(alpha_t, dim=1)
    ...
    def beam_search(self, src_sent: List[str], beam_size: int=5, max_decoding_time_step: int=70) -> List[Hypothesis]:
        ...
        while len(completed_hypotheses) < beam_size and t < max_decoding_time_step:
            ...
            # 为了满足prev_hyp_id是int，所以这里我用了//，而不是原先的/
            ...
            for prev_hyp_id, hyp_word_id, cand_new_hyp_score in zip(prev_hyp_ids, hyp_word_ids, top_cand_hyp_scores):
                ...
                # 之前run test时候报错，这里的prev_hyp_id是float不是int有问题。
                # 为此我修改了之前的一行代码
