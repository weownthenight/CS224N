...
class VocabEntry(object):
    ...
    def words2charindices(self, sents):
        ...
        # 1和2分别对应的char为'{'和'}'是为每个单词添加的'start_of_word'和'end_of_word'
    ...
    def to_input_tensor_char(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        sents2idx = self.words2charindices(sents)

        sents_padded = pad_sents_char(sents2idx, self.char2id['<pad>'])

        # 这里的类型一定得是torch.long,这点参考to_input_tensor也能看出来
        # contiguous
        sents_tmp = torch.tensor(sents_padded, dtype=torch.long, device=device).contiguous()
        # (batch_size, max_sentence_length, max_word_length)
        # reshape和permute结果完全不同，注意区别，这里只能用permute
        sents_var = sents_tmp.permute(1, 0, 2).contiguous()
        return sents_var
        ### END YOUR CODE
