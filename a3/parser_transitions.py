...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        # 一定要加上这一句！！！否则欲哭无泪。因为从minibatch那里看到有可能存在这种情况(因为我们的model就随便给动作，没判断)
        if len(self.buffer) == 0 and len(self.stack) == 1:
            return

        if transition == "S":
            self.stack.append(self.buffer[0])
            # 对buffer不能用del，因为之前定义了buffer=sentence，del buffer会导致sentence变化
...
def minibatch_parse(sentences, model, batch_size):
    ...
    while len(unfinished_parses) != 0:
        ...
        for i in range(len(minibatch)):    # 这里不能写batch_size!!!
            minibatch[i].parse_step(transitions[i])
        for i in range(len(minibatch)):    # 同理，这里也不能写batch_size!!!
