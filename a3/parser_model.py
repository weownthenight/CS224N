...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        # 注意一下这里的input size，每个feature也是embedding的
    ...
    def embedding_lookup(self, t):
        ...
        tmp = self.pretrained_embeddings(t)   # 得到的shape为(batch_size, n_features, emdedding_size)
    ...
    def forward(self, t):
        ...
        # t的shape不是(batch_size,n_features)而是(embed_size,n_features)
        embeded_t = self.embedding_lookup(t)
        hidden_t = self.embed_to_hidden(embeded_t)
        relu_t = nn.functional.relu(hidden_t)
        drop_t = self.dropout(relu_t)
        logits = self.hidden_to_logits(drop_t)
